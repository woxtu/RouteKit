...
class PayloadDecoder {
    ...
    class InnerKeyedDecodingContainer<Key>: KeyedDecodingContainerProtocol where Key: CodingKey {
        ...
        var allKeys: [Key] {
            return decoder.data.keys.compactMap(Key.init)
        }
        ...
        func decodeNil(forKey key: Key) throws -> Bool {
            return decoder.data[key.stringValue] == nil
        }
        ...
        func decode<T>(_ type: T.Type, forKey key: Key) throws -> T where T: Decodable & LosslessStringConvertible {
            ...
            guard let value = T(rawValue) else {
                // swiftlint:disable:next line_length
                throw DecodingError.dataCorrupted(DecodingError.Context(codingPath: codingPath, debugDescription: "String value \"\(rawValue)\" does not fit in \(type)."))
            }
